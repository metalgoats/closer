...
### Logins and roles (TASK-112)

Two roles. The first-run user is `admin`; everyone created afterwards defaults to `member`.

| | admin | member |
|---|---|---|
| Calls, debriefs, outputs, chat | ✅ | ✅ |
| Activity | ✅ | ✅ |
| Prompt Library | ✅ | ✅ |
| **Spend** | ✅ | ❌ |
| **Integrations** (holds the API keys) | ✅ | ❌ |
| **Backups** (`/api/backup` — every transcript) | ✅ | ❌ |
| Creating logins | ✅ | ❌ |

**The boundary is `ADMIN_ONLY` in `src/index.js`, not the menu.** The front end hides those pages
from members so they do not click into a 403; that is cosmetic. If you add an admin page, add it
to `ADMIN_ONLY` first and to `ADMIN_VIEWS` in `public/app.js` second — only the first is a
permission. `tests/worker-refs.test.mjs` fails if the two disagree.

Admins add a login and anyone changes their own password under **Account & Access** in the
profile menu. Changing a password signs out that user's other sessions. `/api/setup` is still
first-run-only.
...
## Auto-processing (TASK-058 / TASK-114)

`AUTO_PROCESS_IMPORTS` in `src/index.js` is **on** as of 2026-08-12. Imported Fathom meetings are
generated automatically, capped at `MAX_AUTO_PROCESS_PER_TICK = 3` per 5-minute tick.

It is safe to have on **only because the poll is scoped by `integrations.owner_email`** — Fathom
returns every meeting on the account, and a token with no owner email is skipped entirely rather
than polled wide. Before turning this on anywhere else, confirm that column is populated:

```bash
CLOUDFLARE_ACCOUNT_ID=... npx wrangler d1 execute closer --remote \
...
first attempt. Retries back off ~10s total, so a momentary rejection recovers by itself and a
sustained one still lands in the inbox as FAILED for a human to re-run.

**It is also a bill.** Over the 14 days to 2026-08-11, 35 calls were imported and 16 were
generated by hand — so auto-processing pays for roughly **2.2x** as many runs. At the Opus 5
default that is ~$28/mo → ~$62/mo for one user; on Sonnet 5 the same volume is ~$10/mo. Turning
it off is one line.
